...
原生书源执行接缝见 [Hub 桥接](hub-bridge.md)，逐项目取舍见 [参考审计](reference-audit.md)。本轮增加声明式插件生成与独立挂载同步，保持控制面不执行第三方规则。Hub 的内容搜索与缓存继续留在 Hub；Relay 通过已核对的管理 API 执行抽检与显式热加载。

`Source` 增加 `probeIntervalMinutes`、`nextProbe`、`smokeKeyword` 与 `hubPluginId`；体检调度独立于版本同步，因此 pinned/manual 源仍可体检。`SourceSet` 增加 `channelRules`、`autoPublish`、`minAvailable`、`maxExcludedPercent`、`nextPublish` 与发布输入指纹。字段保存在既有 JSONB 行中，旧数据默认关闭自动化，无破坏性迁移。

自动编译每 15 分钟检查一次，仅在实际发布输入变化且可用率门槛满足时创建版本。输入指纹排除探测时间等无内容变化的字段，避免每次体检都产生新发布。手工编辑组和频道覆盖携带更新时间，防止覆盖另一会话已提交的编排。

本地文本源可以提交内容新版本，沿用加密原始快照、结构检查、diff、待审和批准流程；远程源通过订阅更新，需独立本地副本才能手工编辑。频道覆盖仅改变编译结果，不修改上游规则或损坏下一次同步。
...
6. `auto` 仅在已有批准版本、删除比例低于 30%、没有新域名、非隔离状态且**新内容功能抽样成功**时批准。仅结构或服务可达检查不足以自动批准；阅读、so-novel 和 Relay recipe 书源包始终需要人工批准新版本。
7. 人工拒绝的相同内容哈希不会在后续同步中重新自动批准。内容变化后才作为新版本再次判断。
8. 发布与批准分开。编排组默认手动发布；管理员显式打开 `autoPublish` 后，按独立周期检查已批准变化与可用率门槛，再让客户端统一切换到完整版本。
