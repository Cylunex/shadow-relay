...
3. **启用源**：只批准仍不会进入发布；也可以使用“批准并启用”一次完成这两个动作，须指定当前待审版本。
...
阅读规则中的 `##` 标记、模板和逗号后的请求选项按原始规则保留，不作为 Relay 的抓取地址校验。阅读格式仍然拒绝嵌入账号密码或敏感凭据字段，并在校验失败时保留旧发布。源包下载地址仍执行严格的 URL 与网络访问校验。
...
| 批准并启用 | `POST /api/v1/sources/{id}/approve-enable` | `{"revision":"REPLACE_STAGED_REVISION_ID"}`；可替代上面的批准和启用两步 |
